...
func updateSQL(dbObj *sql.DB, currentDdlHash md5Hash, tag string) bool {
	if (currentDdlHash == md5Hash{}) {
		log.Info().Msgf("Already the %s seems nuked", tag)
		return false
	}

	data, err := GetMigrateUpdates(currentDdlHash, tag)
	if err != nil {
		log.Fatal().Err(err).Msgf("Failed to get migration updates for %s", tag)
	}

	if data == nil {
		log.Info().Msgf("No migration updates needed for %s", tag)
		return false
	}

	log.Info().Msgf("Migrating new %s ddl updates...", tag)
	_, err = dbObj.Exec(string(data))
	if err != nil {
		log.Warn().Err(err).Msgf("Updating new %s ddl failed.", tag)
		return false
	}
...
func UpdateDDLIfNeeded(dbObj *sql.DB, tag string, ddl []string, hashKey string, noauto bool) {
	...
	if fileDdlHash != currentDdlHash {
		...
		isUpdated := updateSQL(dbObj, currentDdlHash, tag)
		if isUpdated {
			log.Info().Msgf("DDL migrations for %s was successful", tag)
			return
		}
