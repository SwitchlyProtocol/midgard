...
func (a *aggregateDescription) AddJoinQuery(tableName string, alias string) *aggregateDescription {
	a.joinQueries = append(a.joinQueries, joinQuery{
		table: tableName,
		alias: alias,
	})
	return a
...
func (agg *aggregateDescription) baseQueryBuilder(b io.Writer, aggregateTimestamp string, whereConditions []string, groupColumns []string) {
	...
	fmt.Fprintf(b, "\t\tFROM %s", agg.table)
	for _, join := range agg.joinQueries {
		fmt.Fprint(b, ", "+join.table+" AS "+join.alias)
		whereConditions = append(whereConditions, agg.table+".block_timestamp = "+join.alias+".block_timestamp")
	}
	fmt.Fprint(b, "\n")
...
func (agg *aggregateDescription) UnionQuery(timeLow Nano, timeHigh Nano, whereConditions []string, params []interface{}) (string, []interface{}) {
	...
	if timeLow > 0 {
		...
		if timeLowCeil != timeLow {
			...
			agg.baseQueryBuilder(
				&b,
				fmt.Sprintf("MIN(%s.block_timestamp)", agg.table),
				append(
					whereConditions,
					fmt.Sprintf("$%d <= %s.block_timestamp", timeLowParam, agg.table),
					fmt.Sprintf("%s.block_timestamp < $%d", agg.table, timeLowCeilParam)),
	...
	if timeHigh != timeHighFloor {
		...
		agg.baseQueryBuilder(
			...
			append(
				whereConditions,
				fmt.Sprintf("$%d <= %s.block_timestamp", timeHighFloorParam, agg.table),
				fmt.Sprintf("%s.block_timestamp < $%d", agg.table, timeHighParam)),
			agg.groupColumns(false),
		)
		fmt.Fprint(&b, ")\n\tUNION ALL\n")
	}
