...
func setAggregatesStats(
	...
	if !ok {
		earnings = &stat.PoolEarnings{
			Pool: pool,
		}
	}

	poolEarnings := earnings.TotalLiquidityFeesRune + earnings.Rewards
	poolEarningsAnnualPercentage := float64(poolEarnings) * db.GetPPYFromBuckets(buckets) / float64(poolInfo.RuneDepth)
	...
	ret.Earnings = util.IntStr(poolEarnings)
	ret.EarningsAnnualAsPercentOfDepth = floatStr(poolEarningsAnnualPercentage)
