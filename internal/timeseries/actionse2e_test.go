...
func TestAffiliateFieldsDoubleSwap(t *testing.T) {
	...
	blocks.NewBlock(t, "2020-09-01 00:00:01",
		...
		testdb.Outbound{
			...
			InTxID:      "1234",
			Coin:        "1 THOR.RUNE",
			FromAddress: "pool_module",
			ToAddress:   "ouraffiliateaddress",
			Memo:        "OUT:1234",
		},
		testdb.Fee{
			TxID:  "1234",
	...
	require.Equal(t, []oapigen.Action{{
		...
		Out: []oapigen.Transaction{
			...
			{
				Address:   "ouraffiliateaddress",
				Affiliate: util.WrapBoolean(true),
				Coins:     []oapigen.Coin{{Amount: "1", Asset: "THOR.RUNE"}},
				Height:    &outHeight,
			},
		},
		Metadata: oapigen.Metadata{Swap: &oapigen.SwapMetadata{
			LiquidityFee:     "31000", // 10000 + 20000 + 1000
			SwapSlip:         "298",   // 100, 200
			SwapTarget:       "50000",
			NetworkFees:      []oapigen.Coin{{Amount: "2", Asset: "BTC.BTC"}},
			AffiliateFee:     "100",
...
func TestStreamingSwapWithAffiliate(t *testing.T) {
	blocks := testdb.InitTestBlocks(t)
	...
	require.Equal(t, []oapigen.Action{{
		...
		In: []oapigen.Transaction{{
			Address: "eth1",
			Coins:   []oapigen.Coin{{Amount: "200000", Asset: "ETH.ETH"}},
			TxID:    "1234",
		}},
		Metadata: oapigen.Metadata{
			Swap: &oapigen.SwapMetadata{
...
// TODO (HooriRn): Add  Message test
func TestThornameActions(t *testing.T) {
	blocks := testdb.InitTestBlocks(t)
	...
	blocks.NewBlock(t, "2020-09-01 00:00:00",
		testdb.THORName{
			...
			ExpireHeight:    123456,
			TxID:            "123456",
			Memo:            "~:thor1xxxx:thor2xxxx",
			Signer:          thor2,
		},
	)

	body := testdb.CallJSON(t, "http://localhost:8080/v2/actions?type=thorname")
	...
	require.Equal(t, []oapigen.Action{{
		...
		In: []oapigen.Transaction{{
			Address: thor2,
			Coins:   []oapigen.Coin{{Amount: "1100000000", Asset: "THOR.RUNE"}},
			TxID:    "123456",
		}},
		Metadata: oapigen.Metadata{
			Thorname: &oapigen.ThornameMetadata{
				Memo:            "~:thor1xxxx:thor2xxxx",
				Address:         thor1,
				Chain:           "THOR",
				Expire:          "123456",
...
func TestBondActions(t *testing.T) {
	blocks := testdb.InitTestBlocks(t)
	...
	blocks.NewBlock(t, "2020-09-01 00:00:00",
		...
		testdb.Bond{
			...
			From:     thor2,
			Coin:     "1 THOR.RUNE",
			Memo:     "BOND:thor1xxxx:thor3xxx:5000",
			To:       "bond_module",
		},
	)

	blocks.NewBlock(t, "2020-09-01 00:01:00",
		testdb.Bond{
			TxID:     "123458",
			Chain:    "THOR",
			E8:       90000000000000,
			...
			From:     thor2,
			Coin:     "1 THOR.RUNE",
			Memo:     "UNBOND:thor1xxxx:90000000000000",
			To:       "bond_module",
		},
	)

	body := testdb.CallJSON(t, "http://localhost:8080/v2/actions?address=thor1xxxx")
	...
	require.Equal(t, oapigen.Action{
		...
		In: []oapigen.Transaction{{
			Address: thor2,
			Coins:   []oapigen.Coin{{Amount: "90000000000000", Asset: "THOR.RUNE"}},
			TxID:    "123456",
		}},
		Metadata: oapigen.Metadata{
			Bond: &oapigen.BondMetadata{
				Memo:        "BOND:thor1xxxx",
				NodeAddress: thor1,
			},
		},
	...
	require.Equal(t, oapigen.Action{
		...
		Metadata: oapigen.Metadata{
			Bond: &oapigen.BondMetadata{
				Memo:        "BOND:thor1xxxx:thor3xxx:5000",
				NodeAddress: thor1,
				Provider:    util.WrapString("thor3xxx"),
				Fee:         util.WrapString("5000"),
			},
		},
		Out: []oapigen.Transaction{{
			Address: "bond_module",
		...
		Status: "success",
		Type:   "bond",
	}, jsonResult.Actions[1])

	// Unbond
	require.Equal(t, oapigen.Action{
		Date:   util.IntStr(db.StrToSec("2020-09-01 00:01:00").ToNano().ToI()),
		Height: "2",
		In: []oapigen.Transaction{{
			Address: thor2,
			Coins:   []oapigen.Coin{{Amount: "1", Asset: "THOR.RUNE"}},
