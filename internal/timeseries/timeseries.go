...
func Setup() error {
	...
	// set chain info
	poolsStatus, err := GetPoolsStatuses(context.Background(), db.Nano(track.Timestamp.UnixNano()))
	if err != nil {
		return fmt.Errorf("can't get pools status: %w", err)
	}

	for pool, status := range poolsStatus {
		record.Recorder.SetPoolStatus(pool, status)
	}

	mimirs, err := GetAllLastConstantValue(context.Background())
	if err != nil {
		return fmt.Errorf("can't get mimirs: %w", err)
	}

	for k, v := range mimirs {
		record.Recorder.SetMimirStatus(k, v)
	}
...
// result into dest
// Also, if there is no row to be shown it will return the empty interface{} instead of making error
func QueryOneValue(dest interface{}, ctx context.Context, query string, args ...interface{}) error {
	err := db.TheDB.QueryRow(query, args...).Scan(dest)
	if err != nil {
