...
func TestSwapUsdPrices(t *testing.T) {
	...
	require.Equal(t, "2.5", swapHistory.Intervals[1].RunePriceUSD)
	require.Equal(t, epochStr("2020-01-03 00:00:00"), swapHistory.Intervals[2].StartTime)
	require.Equal(t, "2.5", swapHistory.Intervals[2].RunePriceUSD)
	require.Equal(t, "2.5", swapHistory.Meta.RunePriceUSD)
}
...
func TestPoolSwapVolume(t *testing.T) {
	...
	require.Equal(t, "10203440", result.Volume24h)
}
...
func TestPoolsSwapVolume(t *testing.T) {
	...
	require.Equal(t, "10203440", result[0].Volume24h)
}
