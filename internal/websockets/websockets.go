...
var (
	receiveWaitTimer    = timer.NewTimer("websocket_receive_wait")
	receiveProcessTimer = timer.NewTimer("websocket_receive_process")
)
...
func readMessagesWaiting(ctx context.Context) {
	for {
		...
		waitTimer := receiveWaitTimer.One()
		connections, err := connManager.WaitOnReceive()
		waitTimer()

		recieveTimer := receiveProcessTimer.One()
		if err != nil {
			// To be expected...
